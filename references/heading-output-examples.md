...
$p_X(j) = \sum_k p_{X,Y}(j,k)$。表格题通常是“沿列求 $p_X$，沿行求 $p_Y$”。`Lec 3 课件页 20-25｜推荐练习：PS3 Q2`
...
|---|---|---|---|---|---|---|
| 2023 | Q1b | Incentive Compatibility (IC) | In Scope | `Lec 6 课件页 34-42` | `PS2 Q3` | Current lecture covers IC/IR proof structure |
| 2021 | Q4c | Public goods mechanism with topic removed from current slides | Out of Scope | `No current lecture match` | `None found` | Topic does not appear in current lecture index |
...
激励相容 (Incentive Compatibility, IC) and individual rationality (IR).
...
This subquestion is in scope because current Lecture 6 covers IC/IR definitions and the same utility-comparison proof technique appears in PS2 Q3.
...
Public goods mechanism detail not present in the current lecture index.

#### Current Lecture Location

`No current lecture match`
...
This subquestion is out of scope because the tested mechanism does not appear in the current syllabus or current lecture slides. Do not prioritize it unless the user confirms the topic is still examinable.
```
